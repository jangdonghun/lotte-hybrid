	...
	function init_template(){	
		...
		$('a[data-deploy-menu]').on( "click", function(){
			...
			if($(this).hasClass('dismiss-with-button')){} else {
				windowDim_scr = $(document).scrollTop();
				if($('.menu-wrapper').hasClass('active-menu')){$('#page-transitions').addClass('body-fixed');}
				...
				$('#page-transitions').css({ "top": -Number(windowDim_scr) });
			}
		});
		var windowDim_scr;
		$('.delete-menu, .close-menu').on('click', function(){
			...
			$('#page-transitions').removeClass('body-fixed');
			$('#page-transitions').css({ "top": 0 });
			window.scrollTo(0, windowDim_scr);
		...
		setTimeout(function(){
			...
			$('.single-slider-dot-timeout').owlCarousel({loop:true, margin:0, nav:false, items:1, autoHeight:true});
